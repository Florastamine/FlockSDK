...
varying vec2 vScreenPos; 
...
#ifdef COMPILEPS
	uniform float cGlitchScreenWidth;
	uniform float cGlitchScreenHeight;

    uniform float cGlitchdt;
    ...
    {
        return texture(sampler, uv - mod(uv, vec2(pixelSize) / vec2(cGlitchScreenWidth, cGlitchScreenHeight).xy));
    }
    ...
    {
        float dx = sampleSize / cGlitchScreenWidth;
        float dy = sampleSize / cGlitchScreenHeight;
        return (
    ...
    {
        vec2 screenSize = vec2(cGlitchScreenWidth, cGlitchScreenHeight); 
        vec2 pixel = vec2(1.0) / screenSize.xy;
        vec3 field = rgb2hsv(edge(sampler, uv, edgeSize));
        vec2 distort = pixel * sin((field.rb) * M_PI * 2.0);
        float shiftx = noise(vec2(quantize(uv.y + 31.5, cGlitchScreenHeight / TILE_SIZE) * cGlitchdt, fract(cGlitchdt) * 300.0));
        float shifty = noise(vec2(quantize(uv.x + 11.5, cGlitchScreenWidth / TILE_SIZE) * cGlitchdt, fract(cGlitchdt) * 100.0));
...
void VS()
{
    ...
    void PS() 
    {
        ...
        vec3 finalColor;
        finalColor += distort(sDiffMap, uv, 8.0);
