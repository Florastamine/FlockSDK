...
{
    browserWindow = LoadEditorUI("ui/SDK/EditorResourceBrowser.xml");
    ...
    browserFilterWindow = LoadEditorUI("ui/SDK/EditorResourceFilterWindow.xml");
...
void CreateResourcePreview(String path, Node@ previewNode)
{
    ...
    if (resourceType > 0)
    {
        ...
        else if (resourceType == RESOURCE_TYPE_MATERIAL)
        {
            ...
            {
                StaticModel@ staticModel = previewNode.CreateComponent("StaticModel");
                staticModel.model = cache.GetResource("Model", "objects/Sphere.mdl");
        ...
        else if (resourceType == RESOURCE_TYPE_IMAGE)
        {
            ...
            {
                StaticModel@ staticModel = previewNode.CreateComponent("StaticModel");
                staticModel.model = cache.GetResource("Model", "objects/SDK/ImagePlane.mdl");
                Material@ material =  cache.GetResource("Material", "materials/SDK/TexturedUnlit.xml");
    ...
    Material@ material =  cache.GetResource("Material", "materials/SDK/TexturedUnlit.xml");
    Texture2D@ texture = Texture2D();
    Image@ noPreviewImage = cache.GetResource("Image", "textures/SDK/NoPreviewAvailable.png");
